...
def get_times_daily(startdate,enddate, serverroot, wrrsetroot, filename,logger):
    ...
    for thedate in dateList:
        ncfile = serverroot + wrrsetroot + "%d" % (thedate.year) + "/" + filename + "%d_%02d.nc" % (thedate.year,thedate.month)  
...
def getmapname(number,prefix,day):
    ...
    :return: Name
    """
    #Name format: e.g. PM_NM_YEAR_DOY_HR hourly, PM_NM_YEAR_DOY daily
    
    mapname = prefix + '_NM_' + day.strftime('%Y') + '_' + day.strftime('%j') + '.tif'
